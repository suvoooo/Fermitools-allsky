...
gll_psc_v22 = fits.open('gll_psc_v27.fit') # v27 for DR2
...
for i in range(num_cats):
    ...
    choose_number_fsrq = int(len(mock_GLON_fsrq) * random.choice(np.random.uniform(0.84, 0.94, num_cats)))
...
#                   result_list.pop()
    ...
    if (len(c_lp_a_fsrq_comb_bcu) <= 1370) and (len(c_lp_a_fsrq_comb_bcu) >= 980):
        ...
        mock_xmlfile_fsrq_bcu_comb = open('./gen_fsrq/mock_4FGL_V22_fsrq_comb_bcu_file%dS%d.xml' %(source_num_check, source_numbers_check), 'w')
    ...
    source_num_check = source_num_check + 1
