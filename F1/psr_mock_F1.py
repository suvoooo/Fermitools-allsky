...
possible_PSR_lat1 = np.random.normal(-0.5,  3.56443164, 1100)
possible_PSR_lat1_selected = np.random.choice(possible_PSR_lat1, 1050)

possible_PSR_lat2 = np.random.normal(2.16, 2.68265452e+01, 1100)
possible_PSR_lat2_selected = np.random.choice(possible_PSR_lat2, 1050)
...
# print (E100hist_list_psr, '\n', E100bins_psr_list)

print (np.argmax(E100hist_psr), np.max(E100hist_psr), len(E100hist_list_psr), len(E100bins_psr))
print ('minimum bin boundary: ', E100bins_psr[0], E100bins_psr[1])

selected_hist1_psr = E100hist_list_psr[0:6]
 
selected_bins1_psr = E100bins_psr_list[0:6]

# print (selected_hist1_psr[3], '\n', selected_bins1_psr)


first_threshold = 3.5e-13
log_first_threshold = math.log10(first_threshold) 

fermi_threshold = 2.4e-12
log_fermi_threshold = math.log10(fermi_threshold)

# print ('first threshold and fermi threshold log: ', log_first_threshold, log_fermi_threshold)
...
# ax1.axhline(y=selected_hist1_psr[2], xmin=0.05, xmax=0.35, color='grey')
# ax1.set_yscale('log')
...
bin_size_psr = selected_bins1_psr[0]-selected_bins1_psr[1] # same for all bins 
# print ('bin size: ', bin_size_psr)
added_bins_psr = []
for i in range(1,10):
  new_bin = selected_bins1_psr[0] + (bin_size_psr *i)
  # print (new_bin)
  if new_bin >= math.log10(first_threshold):
    added_bins_psr.append(new_bin)
# print (added_bins_psr, '\n', len(added_bins_psr))    

added_bins_psr = added_bins_psr[::-1]

added_bins_psr.extend(E100bins_psr)

# print ('check new bins list: ', added_bins_psr, len(added_bins_psr))


# print (E100hist_psr[2], selected_hist1_psr[2])
E100hist_psr_high_L = E100hist_psr[2:]
# print (E100hist_psr_high_L)

E100hist_list_psr_added = [selected_hist1_psr[2]]*3 
# # multiplication factor = added bins (here it is 3) + from which entry highest counts were considered, here it is 2 
E100hist_list_psr_added.extend(E100hist_psr_high_L)

# print(E100hist_list_psr_added, '\n', len(E100hist_list_psr_added), '\n', len(added_bins_psr))
# print (E100hist_list_psr)
...
for i in range(num_cats):
    ...
    def simple_lum_plec(n, PLEC_i, PLEC_a, PLEC_b, PLEC_FluxD, PLEC_PivEn, PLEC_lat, PLEC_lon):
        ...
        for x in range(choose_number_psr):
            ...
            if result_psr>=10**added_bins_psr[0] and result_psr < 10**(added_bins_psr[-1]):
                ...
                mockE100hist_plec, bins_plec = np.histogram(np.log10(result_list), bins=added_bins_psr, density=False)
                mockE100hist_plec_list = mockE100hist_plec.tolist()
#               for Ebins in E100bins_notlast:
                for x1, x2, x3 in zip(mockE100hist_plec_list, E100hist_list_psr_added, noise_N_PLEC_combined): 
                    if (x1*x3) > x2:
...
#                 result_list.pop()    
    ...
    f_result_list_pl, c_pl_i, c_pl_Expa, c_pl_Expb, c_pl_FD, c_pl_PEn, c_GLAT_pl, c_GLON_pl = simple_lum_plec(1080, 
                                                                                                          rand_PLEC_index, 
    ...
    if (len(c_pl_i)<=450 ) and (len(c_pl_i)>=280):     
