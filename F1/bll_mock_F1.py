...
year_mean_mock_dist3fbll = year_mean_mock_dist3_bll[(year_mean_mock_dist3_bll<7.)]
...
print ('check mean of new var dist: ', np.mean(year_mean_mock_dist3fbll_list_new))
...
E100bins_bll = E100bins_bll.tolist()

print (np.argmax(E100hist_bll), np.max(E100hist_bll), len(E100hist_list_bll), len(E100bins_bll))
print ('minimum bin boundary: ', E100bins_bll[0], E100bins_bll[1])

selected_hist1_bll = E100hist_bll[0:9]
 
selected_bins1_bll = E100bins_bll[0:9]

print (selected_hist1_bll, '\n', selected_bins1_bll)
...
bin_size = selected_bins1_bll[0]-selected_bins1_bll[1] # same for all bins 
print ('bin size: ', bin_size)
added_bins_bll = []
for i in range(1,10):
  new_bin = selected_bins1_bll[0] + (bin_size *i)
  # print (new_bin)
  if new_bin >= math.log10(first_threshold):
    added_bins_bll.append(new_bin)
print (added_bins_bll, '\n', len(added_bins_bll))    

added_bins_bll = added_bins_bll[::-1]

added_bins_bll.extend(E100bins_bll)

print ('check new bins list: ', added_bins_bll, len(added_bins_bll))

print (E100hist_bll[7], selected_hist1_bll[-2])
E100hist_bll_high_L = E100hist_bll[7:]
print (E100hist_bll_high_L)

E100hist_list_bll_added = [selected_hist1_bll[-2]]*10
E100hist_list_bll_added.extend(E100hist_bll_high_L)

print(E100hist_list_bll_added, '\n', len(E100hist_list_bll_added), '\n', len(added_bins_bll))
print ('check the last bin: ', added_bins_bll[0], 10**(added_bins_bll[0]))
...
for i in range(num_cats):
    ...
    noise_N_bll_comb = np.random.uniform(0.7, 1.3, AGNs_bll_bins+3)
    ...
    choose_number_bll = int(len(mock_GLON_bll) * random.choice(np.random.uniform(0.82, 0.92, num_cats)))
    choose_number_bcu = int(len(mock_GLON_bcu) * random.choice(np.random.uniform(0.84, 0.94, num_cats)))
    ...
    # loop over the number of sources (selected in random)
        for x in range(choose_number_bll):
            ...
            # 10**(lbin), 10**(hbin)
            if result_bll >= 10**added_bins_bll[0] and result_bll <= 10**(added_bins_bll[-1]):
                ...
                mockE100hist_bll, bins_bll = np.histogram(
                    np.log10(result_list_bll), bins=added_bins_bll, density=False)
                mockE100hist_list_bll = mockE100hist_bll.tolist()
#               for Ebins in E100bins_notlast:
                for x1, x2, x3 in zip(mockE100hist_list_bll, E100hist_list_bll_added, noise_N_bll_comb):
                    if (x1*x3) > x2:
                        result_list_bll.pop()
...
#               for Ebins in E100bins_notlast:
                for x1bcu, x2bcu, x3bcu in zip(mockE100hist_list_bcu, E100hist_list_bcu, noise_N_bcu):
        ...
        for n, al, be, pivE, ra, dec, num in zip(variability_bll_bcu_F, c_lp_a_bll_comb_bcu, c_lp_b_bll_comb_bcu, 
            ...
            if n > 0.0:    
                ...
                mock_xmlfile_bll_bcu_comb.write(
                    '<parameter free="1" max="{0}" min="{1}" name="norm" scale="1.0" value="{2}"/>\n'.format(max(variability_bll_bcu_F), min(variability_bll_bcu_F), n))
                mock_xmlfile_bll_bcu_comb.write(
