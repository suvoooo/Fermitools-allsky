...
v22LP_df_bll = v22LP_df_flux_history[(v22LP_df_flux_history['Class1']=='bll  ') | (v22LP_df_flux_history.Class1=='BLL  ')]
print ('bll num: ', v22LP_df_bll.shape) 

v22LP_df_fsrq = v22LP_df_flux_history[(v22LP_df_flux_history['Class1']=='fsrq ') | (v22LP_df_flux_history.Class1=='FSRQ ')]
print ('fsrq num:', v22LP_df_fsrq.shape)


v22LP_df_bcu = v22LP_df_flux_history[(v22LP_df_flux_history['Class1']=='bcu  ') | (v22LP_df_flux_history.Class1=='BCU  ')]
print ('bcu num:', v22LP_df_bcu.shape)
...
E100bins_fsrq = E100bins_fsrq.tolist()

print (np.argmax(E100hist_fsrq), np.max(E100hist_fsrq), len(E100hist_list_fsrq), len(E100bins_fsrq))
print ('minimum bin boundary: ', E100bins_fsrq[0], E100bins_fsrq[1])

selected_hist1_fsrq = E100hist_fsrq[0:6]
 
selected_bins1_fsrq = E100bins_fsrq[0:6]

print (selected_hist1_fsrq, '\n', selected_bins1_fsrq)

# setting up first threshold 3.4e-13 erg cm^-2 s^-1
...
# ax1.axhline(y=selected_hist1_fsrq[-2], xmin=0.05, xmax=0.35, color='grey')
# ax1.set_yscale('log')
# ax1.set_xlim(-12.80, -8.7)


# ax1.set_xlabel(r'$E_{100}\, \left[\mathrm{erg}\, \mathrm{cm}^{-2}\, \mathrm{s}^{-1}\right] $')
# ax1.set_ylabel('Source Counts')
# ax1.legend(fontsize=10)

### extend the bin boundaries (only the lower part)

bin_size_fsrq = selected_bins1_fsrq[0]-selected_bins1_fsrq[1] # same for all bins 
print ('bin size: ', bin_size_fsrq)
added_bins_fsrq = []
for i in range(1,10):
  new_bin = selected_bins1_fsrq[0] + (bin_size_fsrq *i)
  # print (new_bin)
  if new_bin >= math.log10(first_threshold):
    added_bins_fsrq.append(new_bin)
print (added_bins_fsrq, '\n', len(added_bins_fsrq))    

added_bins_fsrq = added_bins_fsrq[::-1]

added_bins_fsrq.extend(E100bins_fsrq)

print ('check new bins list: ', added_bins_fsrq, len(added_bins_fsrq))

print (E100hist_fsrq[4], selected_hist1_fsrq[-2])
E100hist_fsrq_high_L = E100hist_fsrq[4:]
print (E100hist_fsrq_high_L)

E100hist_list_fsrq_added = [selected_hist1_fsrq[-2]]*7 
# multiplication factor = added bins (here it is 3) + from which entry highest counts were considered, here it is 4 
E100hist_list_fsrq_added.extend(E100hist_fsrq_high_L)

print(E100hist_list_fsrq_added, '\n', len(E100hist_list_fsrq_added), '\n', len(added_bins_fsrq))
...
E100bins_bcu = E100bins_bcu.tolist()
...
num_cats = 1  # num_cats is number of catalog we would like to generate.
start_source_num = 0
...
for i in range(num_cats):
    ...
    noise_N_fsrq_comb = np.random.uniform(0.7, 1.3, AGNs_fsrq_bins+3)
...
# 
        ...
        # 
            ...
            if result_fsrq>= 10**(added_bins_fsrq[0]) and result_fsrq <= 10**(added_bins_fsrq[-1]) : # 10**(lbin), 10**(hbin)
...
# 
            ...
            # 
                mockE100hist_fsrq, bins_fsrq = np.histogram(np.log10(result_list_fsrq), bins=added_bins_fsrq, density=False)
                mockE100hist_list_fsrq = mockE100hist_fsrq.tolist()
                for x1, x2, x3 in zip(mockE100hist_list_fsrq, E100hist_list_fsrq_added, noise_N_fsrq_comb): 
                    if (x1*x3) > x2:
...
#                 result_list.pop()    
    ...
    def simple_lum_bcu_comb(n, LPalpha_bcu, LPbeta_bcu, LPFd_bcu, LPPEn_bcu, LPglat_bcu, LPglon_bcu):
        ...
        for y in range(choose_number_bcu):
            ...
            # 10**(lbin), 10**(hbin)
            if result_bcu >= 10**E100bins_bcu[0] and result_bcu < 10**(E100bins_bcu[-1]):
                ...
                mockE100hist_bcu, bins_bcu = np.histogram(
                    np.log10(result_list_bcu), bins=E100bins_bcu, density=False)
                mockE100hist_list_bcu = mockE100hist_bcu.tolist()
#               for Ebins in E100bins_notlast:
                for x1bcu, x2bcu, x3bcu in zip(mockE100hist_list_bcu, E100hist_list_bcu, noise_N_bcu_comb):
...
#                   result_list.pop()
    ...
    c_lp_a_fsrq_comb_bcu = c_lp_a_fsrq + c_lp_a_bcu_comb_fsrq
    c_lp_b_fsrq_comb_bcu = c_lp_b_fsrq + c_lp_b_bcu_comb_fsrq
